...
def test(file_path):
    ...
    print('\n -*- Features -*-')
    for i in sorted(zip(s.features_field, trainer.model.coef_), key=lambda x: abs(x[1])):
        print(i)
...
if __name__ == "__main__":
    ...
    parser.add_argument('command', metavar='command', help='Sub-command',
                        choices=['test', 'train', 'als', 'als_libmf', 'age_diff', 'add_tmdb', 'populate', 'predict'])
    parser.add_argument('file_path', metavar='file_path', nargs='?', default=r'./Data/ground truth.exp1.csv',
