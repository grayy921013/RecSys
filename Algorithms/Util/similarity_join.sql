CREATE OR REPLACE FUNCTION public.similarity_join()
 RETURNS void
 LANGUAGE plpgsql
AS $function$
BEGIN
    ...
    FROM (
          ...
          UNION
          SELECT  id1_id, id2_id FROM mainsite_SimilarityTags
          UNION
    ...
    FROM TMP_MOVIES_PAIR as a
      ...
      LEFT OUTER JOIN mainsite_SimilarityTags         as p on a.id1_id = p.id1_id and a.id2_id = p.id2_id
     LEFT OUTER JOIN mainsite_Similarityals          as q on a.id1_id = q.id1_id and a.id2_id = q.id2_id
     LEFT OUTER JOIN mainsite_Similaritylibmf          as r on a.id1_id = r.id1_id and a.id2_id = r.id2_id;
    INSERT INTO mainsite_similarity(
            id1_id, id2_id, 
            title_tfitf, title_bm25, title_jaccard,
            genre_tfitf, genre_bm25, genre_jaccard,
            released_tfitf, released_bm25, released_jaccard,
            director_tfitf, director_bm25, director_jaccard,
            writer_tfitf, writer_bm25, writer_jaccard,
            cast_tfitf, cast_bm25, cast_jaccard,
            metacritic_tfitf, metacritic_bm25, metacritic_jaccard,
            plot_tfitf, plot_bm25, plot_jaccard,
            full_plot_tfitf, full_plot_bm25, full_plot_jaccard,
            language_tfitf, language_bm25, language_jaccard,
            country_tfitf, country_bm25, country_jaccard,
            awards_tfitf, awards_bm25, awards_jaccard,
            last_updated_tfitf, last_updated_bm25, last_updated_jaccard,
            filtered_plot_tfitf, filtered_plot_bm25, filtered_plot_jaccard,
            tags_tfitf, tags_bm25, tags_jaccard,
            als_cosine, libmf_cosine)
     SELECT id1_id, id2_id, 
            title_tfitf, title_bm25, title_jaccard,
            genre_tfitf, genre_bm25, genre_jaccard,
            released_tfitf, released_bm25, released_jaccard,
